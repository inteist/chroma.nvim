...
- **Smart Target Detection**: Opens seeded with the color literal under your cursor or visual selection (supports CSS hex, rgb, rgba, argb, hsl, hsla, hsv, prefixed RGB and parenthesized RGBA/ARGB numeric tuples with arbitrary prefixes, and standard names).
...
- **Input formats**: `#rrggbb`, `#rrggbbaa`, `0xrrggbb`, `0xaarrggbb`, `rgb()`, `rgba()`, `argb()`, prefixed RGB tuples (`Color(255, 255, 255)`), parenthesized RGBA tuples (`(255, 255, 255, 0.8)`, `Color(217, 217, 217, 1)`) and ARGB tuples (`(0.3, 255, 255, 255)`, `make_color(0, 217, 217, 217)`) regardless of prefix, `hsl()`, `hsla()`, `hsv()`, and standard CSS color names.
- **Output formats**: `hex` (`#rrggbb`), `hexa` (`#rrggbbaa`), `rgb0x` (`0xrrggbb`), `argb0x` (`0xaarrggbb`), `rgb`, `rgba`, `argb`, `hsl`, `hsla`, `hsv`.
